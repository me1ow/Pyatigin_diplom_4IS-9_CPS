                ...
                <div class="mt-6">
                    ...
                    @foreach($course->modules as $index => $module)
                        <div id="tab-{{ $index }}" class="tab-content mt-6" style="display: {{ $index == 0 ? 'block' : 'none' }}">
                            ...
                            <div class="mt-4">
                                <a href="{{ route('modules.show', $module) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-500">
                                    Перейти к модулю →
                                </a>
                            </div>
                        </div>
